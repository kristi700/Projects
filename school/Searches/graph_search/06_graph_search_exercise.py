...
def _get_heuristic_value(state: State, f) -> int:
    return f(state)
...
def graph_search(problem, f) -> tuple[list[Actions], list[State]]:
    def search_algorithm(problem, f) -> tuple[Optional[State], 
        ...
        G : set[State] = {start}  # startnode
        g : dict[State,int] = f.g
        g[start] = 0 # cost gstart 0
        OPEN = {start} # set containing startnode
        pi : dict[State,Optional[State]] = {start: None} # dictionary to keep track of parent of nodes
        expanded_nodes : list[State] = [] # list to store nodes expanded during search
        while True:
            if not OPEN:
                # return the goal node, the dict of parents and expanded nodes
                return None, pi, expanded_nodes #line3-4 within the slides
            n = min(OPEN, key=lambda state: _get_heuristic_value(state, f))

            if problem.is_goal_state(n): #if goal(n) then return solution (n, pi)
                return n, pi, expanded_nodes
            
            OPEN.remove(n)
            expanded_nodes.append(n)

            for m in problem.next_states(n):
                if m not in G or g[n] + 1 < g.get(m):
                    pi[m] = n
                    g[m] = g[n] + 1
                    OPEN.add(m)
            G.add(n)

            # Idea: Pseudocode from lecture 5 (graphsearch) on slide 8
...
def manhattan(problem: SearchProblem):
    ...
    def heuristics(state: State):
        ...
        # NOTE - by each position I understand that summed up - or was it supposed to be a list?
        # Idea: The sum of absolute distance (from target position) vertically + horisontally (for each target_position)
...
class EatDonutsProblem(SearchProblem):
    ...
    def __init__(self, game_logic: GameLogic):
        cat_position = game_logic.collect_positions(LabyrinthFields.Cat)[0]
        self.donut_positions = game_logic.collect_positions(LabyrinthFields.Donut)
        self.state = EatDonutsState(cat_position, self.donut_positions)
        self.door_positions = game_logic.collect_positions(LabyrinthFields.Door)
        self.walls = [
            [field == LabyrinthFields.Wall for field in row]
            for row in game_logic.board
